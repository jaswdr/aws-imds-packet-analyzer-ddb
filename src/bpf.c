...
#pragma clang diagnostic ignored                                               \
    "-Wduplicate-decl-specifier" // for now we acknowledge the kernel 6.12+
                                 // warnings
...
/*
 * Removed forward declarations that were causing redefinition errors on some
 * kernels. If you are on Kernel 6.12+ and see missing type errors, uncomment
 * them.
 */
// struct bpf_wq {};
// struct bpf_rb_root {};
...
int trace_sock_sendmsg(struct pt_regs *ctx) {
  ...
  if (sk->__sk_common.skc_daddr == IP_169_254_169_254) {
    ...
    // however if we were to intuitively use the size of the payload itself as
    // the size parameter by using iovlen (size of the payload) the interpreter
    // will throw an invalid mem access error and the script will not run (most
    // probably due to the method requiring a const value at compile time)
    bpf_probe_read_str(data->pkt, sizeof(data->pkt), iovbase);

    // check if payload is empty or not -> check char buffer -> if char buffer
    // starts with a termination vales \0 => null buffer
    if (data->pkt[0] == '\0') {
