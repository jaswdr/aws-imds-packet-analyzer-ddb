...
install_ubuntu_bcc() {
  ...
  apt install -y bison build-essential cmake flex git libedit-dev "libllvm${llvm_version}" \
    ...
    libfl-dev python3-distutils zip python3-boto3 --no-install-recommends
...
"amzn2023")
  dnf install -y bcc-tools python3-boto3
  ;;
"amzn2")
  check_sudo_user_home
  amazon-linux-extras enable BCC
  yum -y install "kernel-devel-$(uname -r)" bcc-devel python3-boto3
  ;;
"debian11")
  add_debian_repo
  export DEBIAN_FRONTEND=noninteractive
  apt-get update
  apt-get install -y bpfcc-tools libbpfcc libbpfcc-dev "linux-headers-$(uname -r)" bcc python3-boto3 --no-install-recommends
  ;;
"debian10")
  add_debian_repo
  export DEBIAN_FRONTEND=noninteractive
  apt-get update
  apt-get install -y bpfcc-tools libbpfcc libbpfcc-dev "linux-headers-$(uname -r)" bcc python3-boto3 --no-install-recommends || true
  cd /tmp
...
"ubuntu20.04")
  ...
  ;;
"rhel"*)
  yum -y install bcc-tools libbpf python3-boto3
  ;;
"fedora"*)
  yum -y install bcc-tools libbpf python3-boto3
  ;;
"sles15"*)
  zypper ref
  zypper in -y bcc-tools bcc-examples python3-boto3
  ...
  ;;
"centos"*)
  yum -y install bcc python3-boto3
  ;;
*)
