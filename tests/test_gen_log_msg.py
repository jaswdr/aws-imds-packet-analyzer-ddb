...
def test_single_file():
    with mock.patch("builtins.open", mock.mock_open(read_data="test")):
        assert (
            gen_log_msg(False, Event())
            == "IMDSv1(!) (pid:1:curl argv:test) called by -> (pid:2:zsh argv:test) -> (pid:3:sshd argv:test) -> (pid:3:sshd argv:test)"
        )
...
def test_multiple_files():
    with mock.patch("builtins.open", my_open):
        assert (
            gen_log_msg(False, Event())
            == "IMDSv1(!) (pid:1:curl argv:test1) called by -> (pid:2:zsh argv:test2) -> (pid:3:sshd argv:test3) -> (pid:3:sshd argv:test4)"
        )
